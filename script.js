...
// ============================================================
// DATABASE  –  always use try/finally so syncing resets
// ============================================================
...
async function loadFromDB() {
  ...
  setSyncing(true);
  try {
    // workouts + sleep in parallel (both tables are guaranteed to exist)
    ...
    // routes – fetch separately so a missing table doesn't break workouts/sleep
    const rRes = await db.from('routes').select('*').eq('user_id', currentUser.id).order('created_at', { ascending: false });
    if (!rRes.error && rRes.data) {
      routeEntries = rRes.data.map(row => ({ id: row.id, grade: row.grade, flash: !!row.flash, date: row.created_at }));
    } else if (rRes.error) {
      console.warn('routes fetch error:', rRes.error.message);
    }

    renderAll();
  } catch (err) {
    if (workouts.length === 0 && sleepData.length === 0) showToast('⚠️ Offline – keine Daten verfügbar');
    renderAll();
  } finally {
    setSyncing(false);
...
async function deleteWorkoutFromDB(id) {
  if (!currentUser) return;
  setSyncing(true);
  try { await db.from('workouts').delete().eq('id', id); } finally { setSyncing(false); }
}
...
async function saveRouteToDB(entry) {
  ...
  try {
    ...
    if (error) {
      ...
      console.error('saveRouteToDB:', error);
      return entry;
    }
    return { ...entry, id: data.id, date: data.created_at };
  } catch (err) {
    showToast('⚠️ Route-Fehler: ' + err.message, 4000);
    return entry;
  } finally {
    setSyncing(false);
  }
...
// per-view stepper values & selected date
const viewSteppers = {};
const viewDates    = {}; // selected date per exercise view
...
// ---- FORM (inline steppers + date picker + save button) ----
...
function viewStep(exId, fId, delta) {
  ...
  if (el) {
    el.textContent = fmtVal(val, field);
    el.style.animation = 'none';
    requestAnimationFrame(() => { el.style.animation = 'popIn 0.18s ease'; });
  }
}
...
async function addEntry(exId) {
  ...
  let workout = {
    id:           Date.now(),
    date:         viewDates[exId] + 'T12:00:00.000Z',
    exerciseId:   ex.id,
...
function renderExercisePR(exId, viewKey) {
  ...
  const parts   = [];
  if (allSets.some(s => s.reps > 0))
    parts.push(`<span class="pr-val">${Math.max(...allSets.map(s=>s.reps||0))}</span><span class="pr-unit"> reps</span>`);
  if (allSets.some(s => s.duration > 0))
    parts.push(`<span class="pr-val">${Math.max(...allSets.map(s=>s.duration||0))}</span><span class="pr-unit"> sek</span>`);
  if (allSets.some(s => s.weight > 0))
    parts.push(`<span class="pr-val">${Math.max(...allSets.map(s=>s.weight||0))}</span><span class="pr-unit"> kg</span>`);
  el.innerHTML = `
    ...
    <div class="card">
      ...
      <div class="chart-wrap">
        <svg width="${svgW}" height="${svgH}" viewBox="0 0 ${svgW} ${svgH}">
          ...
          <path d="${pathD} L${last.x.toFixed(1)},${svgH-22} L${coords[0].x.toFixed(1)},${svgH-22} Z"
                fill="url(#lg_${viewKey})"/>
          <path d="${pathD}" fill="none" stroke="var(--accent)" stroke-width="2.5"
                stroke-linecap="round" stroke-linejoin="round"/>
          ${dots}
          <text x="${last.x.toFixed(1)}" y="${(last.y-9).toFixed(1)}"
                text-anchor="middle" fill="var(--gold)" font-size="9" font-family="Inter" font-weight="700">${last.val}</text>
          <line x1="14" y1="${svgH-22}" x2="${svgW-14}" y2="${svgH-22}"
                stroke="rgba(255,255,255,0.08)" stroke-width="1"/>
          ${coords.filter((_,i)=>i%step===0).map(c=>
...
function renderSleepChart(data) {
  const svgW=Math.max(300,(window.innerWidth||400)-52), svgH=100;
  const max=Math.max(...data.map(d=>d.value),8), n=data.length;
  const barW=Math.floor((svgW-20)/Math.max(n,1))-2;
  const bars=data.map((d,i)=>{
    const barH=Math.round((d.value/max)*68), x=10+i*(barW+2), y=svgH-22-barH;
  ...
  return `<svg width="${svgW}" height="${svgH}" viewBox="0 0 ${svgW} ${svgH}">
    ...
    <text x="${svgW-12}" y="${refY-3}" text-anchor="end" fill="#FFD60A" font-size="7" opacity="0.5">8h</text>
...
function gradeColor(total,hasFlash){
  if(hasFlash&&total===1) return '#FFD700';
  if(total>=10) return '#FF00FF';
  if(total>=7)  return '#FF3399';
  if(total>=5)  return '#FF6B35';
  if(total>=3)  return '#FFD60A';
  if(total>=2)  return '#30D158';
  return '#6699CC';
}
function fillPct(total,flash){ return Math.min(100, total*8+flash*14); }
...
function renderRouteWall() {
  ...
  if(!g) return;
  const counts={};
  routeEntries.forEach(r=>{ if(!counts[r.grade]) counts[r.grade]={total:0,flash:0}; counts[r.grade].total++; if(r.flash) counts[r.grade].flash++; });
  const done=GRADE_ORDER.filter(gr=>counts[gr]);
  const empty=GRADE_ORDER.filter(gr=>!counts[gr]);
  let svg='';
  done.forEach((grade,idx)=>{
    if(idx>=QD_POS.length) return;
    const pos=QD_POS[idx], {total,flash}=counts[grade];
    const col=gradeColor(total,flash>0), bw=46, fw=Math.max(2,Math.round(bw*fillPct(total,flash)/100));
    svg+=`<g transform="translate(${pos.x-23},${pos.y-30})">
  ...
  empty.slice(0,Math.max(0,QD_POS.length-done.length)).forEach((_,i)=>{
    const pos=QD_POS[done.length+i]; if(!pos) return;
    svg+=`<g transform="translate(${pos.x-8},${pos.y-22})" opacity="0.12">
...
function coachTipHTML(t) {
  return `<div class="coach-tip"><span class="coach-tip-icon">${t.icon}</span><div><strong>${t.title}</strong><div class="coach-tip-text">${t.text}</div></div></div>`;
}
function renderSleepCoach() {
  const el=document.getElementById('sleepCoachContent'); if(!el) return;
  const tips=getSleepTips();
  el.innerHTML=tips.length ? tips.map(coachTipHTML).join('') : `<div class="coach-tip-empty">Tracke deinen Schlaf – dann gibt es hier personalisierte Tipps!</div>`;
}
function getSleepTips() {
  ...
  const tips=[];
  if(avg<7) tips.push({icon:'😴',title:'Mehr Schlaf!',text:`Ø ${avg.toFixed(1)}h – Kletterathlet:innen brauchen 7.5–9h. Fingersehnen regenerieren hauptsächlich im Schlaf.`});
  else if(avg>=8) tips.push({icon:'🌟',title:'Exzellente Basis!',text:`${avg.toFixed(1)}h Ø-Schlaf – top! Nutze diese Energie für intensive Trainingseinheiten.`});
  if(avgQ<=2.5) tips.push({icon:'🌿',title:'Qualität verbessern',text:'Kein Bildschirm 1h vor dem Schlafen, Zimmer 16–18°C, kein Training 3h vor dem Einschlafen.'});
  if(score>=80) tips.push({icon:'💪',title:'Perfekter Trainingstag!',text:'Dein Score ist stark – heute ideal für ein Max-Effort Workout.'});
  return tips;
}
function renderExerciseCoach(exId, viewKey) {
  const el=document.getElementById(viewKey+'-coach'); if(!el) return;
  const tips=getExerciseTips(exId);
  el.innerHTML=tips.length ? tips.map(coachTipHTML).join('') : `<div class="coach-tip-empty">Logge mehr Einträge für personalisierte Tipps!</div>`;
}
function getExerciseTips(exId) {
  const data=workouts.filter(w=>w.exerciseId===exId); if(data.length<2) return [];
  const allSets=data.flatMap(w=>w.sets||[]);
  const recent7=data.filter(w=>(Date.now()-new Date(w.date))/86400000<=7);
  const tips=[];
  if(exId==='pullups'){
    const max=Math.max(...allSets.map(s=>s.reps||0),0);
    if(max>=15)    tips.push({icon:'⚖️',title:'Zeit für Gewicht!',text:`${max} Reps – starte mit +5kg für 5 saubere Reps.`});
    else if(max<5) tips.push({icon:'🎯',title:'Aufbauphase',text:'Fokus auf negative Klimmzüge und 3–5 Sets täglich.'});
    else           tips.push({icon:'📈',title:'Gut dabei!',text:`Max. ${max} Reps – steigere schrittweise um 1–2 Reps pro Woche.`});
  }
  if(exId==='hangboard'){
    const max=Math.max(...allSets.map(s=>s.duration||0),0);
    if(max>=20) tips.push({icon:'🏋️',title:'Gewicht hinzufügen',text:`${max}s – versuche schrittweise +2.5kg.`});
    else        tips.push({icon:'🪨',title:'Fingerboard Grundsatz',text:'7–10sek auf 20mm Leiste mit Pause. Qualität vor Quantität.'});
  }
  if(exId==='deadhang'){
    const max=Math.max(...allSets.map(s=>s.duration||0),0);
    tips.push({icon:'⏱️',title:'Dead Hang Ziel',text:`Dein Rekord: ${max}s. Pro-Ziel: 60s mit Körpergewicht.`});
  }
  if(exId==='lsit'){
    const max=Math.max(...allSets.map(s=>s.duration||0),0);
    if(max<10) tips.push({icon:'💡',title:'L-Sit Aufbau',text:'Starte mit Tuck-L-Sit (Knie angezogen). Ziel: 10s gehalten.'});
    else       tips.push({icon:'🔥',title:`Starker Kern – ${max}s!`,text:'Versuche L-Sit auf Ringen für mehr Schulteraktivierung.'});
  }
  if(recent7.length>=4)  tips.push({icon:'⚠️',title:'Erholung!',text:'Viele Einheiten diese Woche – plane 1–2 Ruhetage ein.'});
  else if(recent7.length===0&&data.length>0) tips.push({icon:'🔥',title:'Zurück ans Training!',text:'Diese Woche noch nichts – selbst 15 Min halten die Kraft.'});
  return tips;
}
